...
module RefinedInputPoint
  refine Sketchup::InputPoint do
    ...
    def freedom_constraint
      ...
      when 1
        return edge_line if source_edge
        axis
      when 2
        source_face && face_normal
      end
    end
    ...
    # Get the direction of the picked edge in model coordinates.
    #
    # @note Only valid when InputPoint is on an edge.
    #
    # @return [Geom::Vetor3d]
    def edge_direction
      edge.line[1].transform(transformation)
    end

    # Get the normal of the picked face in model coordinates.
    #
    # @note Only valid when InputPoint is on a face.
    #
    # @return [Geom::Vetor3d]
    def face_normal
      transform_as_normal(face.normal, transformation)
    end
